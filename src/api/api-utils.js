...
class ApiUtils {
  ...
  // payload 例: {championId: 1, completed: true}，completed 為 true 代表直接鎖定
  static patchChampSelectAction(actionId, payload) {
    return axios.patch(`${ApiUtils.getApiBaseUrl()}/lol-champ-select/v1/session/actions/${actionId}`, payload)
  }
  ...
  static getCurrentRunePage() {
    return axios.get(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/currentpage`)
  }

  static getRunePages() {
    return axios.get(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/pages`)
  }

  // payload 例: {name, isTemporary: true, primaryStyleId, subStyleId, selectedPerkIds: [9 個], current: true}
  static postRunePage(payload) {
    return axios.post(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/pages`, payload)
  }

  static putRunePage(pageId, payload) {
    return axios.put(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/pages/${pageId}`, payload)
  }

  // 回傳 {championId: {recommendedPositions: ['TOP', ...]}}，用來推測敵方英雄走哪一路
  static getRecommendedChampionPositions() {
    return axios.get(`${ApiUtils.getApiBaseUrl()}/lol-perks/v1/recommended-champion-positions`)
  }

  // 英雄最主要的位置(例: 'TOP')，position 給 NONE 時用戶端會回傳預設位置的推薦符文
  static async getChampionDefaultPosition(championId) {
    const response = await axios.get(
      `${ApiUtils.getApiBaseUrl()}/lol-perks/v1/recommended-pages/champion/${championId}/position/NONE/map/11`);
    const page = (response.data ?? []).find(item => item.isDefaultPosition);
    return page?.position ?? null;
  }
