...
const LOL_POLL_INTERVAL_MS = 5000;

  logger.info('[initLolListener] 開始檢測 LOL 是否執行...')
  const credentials = await LeagueClient.authenticate({
  // 找不到用戶端時，每次輪詢都會開 1~2 個 PowerShell，遊戲中用戶端會被關掉，所以間隔不要太短
    awaitConnection: true,
    pollInterval: LOL_POLL_INTERVAL_MS,
  });
...
initUpdater(ipcMain, () => win, () => quitApp(1500));
